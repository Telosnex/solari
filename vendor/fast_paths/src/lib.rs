...
/// Like `prepare()`, but consumes the input graph so its memory can be released before
/// contraction proceeds.
pub fn prepare_owned(input_graph: InputGraph) -> FastGraphVec {
    FastGraphBuilder::build_owned(input_graph)
}
...
pub fn calc_weight_multiple_sources_and_targets<G: FastGraph>(
    ...
    let mut calc = PathCalculator::new(fast_graph.get_num_nodes());
    calc.calc_weight_multiple_sources_and_targets(fast_graph, sources, target)
}
