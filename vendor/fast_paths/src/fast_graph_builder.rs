...
impl ProgressReporter {
    fn new(label: &'static str, total: usize) -> Self {
        let now = Instant::now();
        eprintln!("[fast_paths] {} started total={}", label, total);
        Self {
            label,
            total,
            start: now,
    ...
    fn should_log(&self, completed: usize) -> bool {
        completed >= self.total || self.last_log.elapsed() >= Duration::from_secs(10)
    }

    fn log(&mut self, completed: usize, details: String) {
        ...
        if completed > 0 && completed < self.total {
            let remaining_units = self.total - completed;
            let eta = Duration::from_secs_f64(
                elapsed.as_secs_f64() * remaining_units as f64 / completed as f64,
            );
            eprintln!(
                "[fast_paths] {} progress: {}/{} ({:.1}%) elapsed={} eta={} {}",
                self.label,
                completed,
                self.total,
                pct,
...
impl FastGraphBuilder {
    ...
    pub fn build_owned(input_graph: InputGraph) -> FastGraphVec {
        FastGraphBuilder::build_with_params_owned(input_graph, &Params::default())
    }
    ...
    pub fn build_with_params_owned(input_graph: InputGraph, params: &Params) -> FastGraphVec {
        let mut builder = FastGraphBuilder::new(&input_graph);
        let preparation_graph = PreparationGraph::from_input_graph(&input_graph);
        drop(input_graph);
        builder.run_contraction_prepared(preparation_graph, params);
        builder.fast_graph
    }
    ...
    fn run_contraction(&mut self, input_graph: &InputGraph, params: &Params) {
        let preparation_graph = PreparationGraph::from_input_graph(input_graph);
        self.run_contraction_prepared(preparation_graph, params);
    }

    fn run_contraction_prepared(&mut self, mut preparation_graph: PreparationGraph, params: &Params) {
        ...
        let mut rank = 0;
        let mut progress = ProgressReporter::new("node contraction", self.num_nodes);
        while !queue.is_empty() {
            ...
            rank += 1;
            if progress.should_log(rank) {
                progress.log(
                    rank,
                    format!(
                        "queue_remaining={} edges_fwd={} edges_bwd={}",
                        queue.len(),
                        self.fast_graph.get_num_out_edges(),
                        self.fast_graph.get_num_in_edges()
                    ),
    ...
    fn run_contraction_with_order(
        ...
        let mut witness_search = WitnessSearch::new(self.num_nodes);
        let mut progress = ProgressReporter::new("node contraction with fixed order", self.num_nodes);
        for (rank, node) in order.iter().cloned().enumerate() {
            ...
            let completed = rank + 1;
            if progress.should_log(completed) {
                progress.log(
                    completed,
                    format!(
                        "edges_fwd={} edges_bwd={}",
    ...
    fn finish_contraction(&mut self) {
        let mut fwd_progress = ProgressReporter::new("forward edge finalization", self.num_nodes);
        for i in 0..self.num_nodes {
            ...
            let completed = i + 1;
            if fwd_progress.should_log(completed) {
                fwd_progress.log(
                    completed,
                    format!("edges_fwd={}", self.fast_graph.get_num_out_edges()),
                );
            }
        }

        let mut bwd_progress = ProgressReporter::new("backward edge finalization", self.num_nodes);
        for i in 0..self.num_nodes {
            ...
            let completed = i + 1;
            if bwd_progress.should_log(completed) {
                bwd_progress.log(
                    completed,
                    format!("edges_bwd={}", self.fast_graph.get_num_in_edges()),
                );
            }
        }
    }
